DROP
DATABASE IF EXISTS `buyme`;
CREATE
DATABASE IF NOT EXISTS `buyme`;
...
-- insert for car
LOCK
TABLES auctionItem WRITE;
INSERT INTO auctionItem (productID, type, listPrice, minSellPrice, exteriorColor, interiorColor, model, manufacturer,
                         `condition`, capacity, closingDate, `year`, listDate, seller, purchaser)
VALUES ('acc', 'Car', 12000.00, 18000.00, 'white', 'black', 'Gappu', 'Volkswagon', 'used', 5, '2022-05-01 12:30:00',
        2015, '2021-04-11 16:30:00', 6, 5);
UNLOCK
TABLES;
...
VALUES ('acd', 'Car', 12000.00, 18000.00, 'white', 'black', 'Pasat', 'Volkswagon', 'used', 5, '2022-05-01 12:30:00',
        2015, '2021-04-11 16:30:00', 6);
UNLOCK
TABLES;
...
INSERT INTO auctionItem (productID, type, listPrice, minSellPrice, exteriorColor, interiorColor, model, manufacturer,
                         `condition`, capacity, closingDate, `year`, listDate, seller)
VALUES ('adc', 'Boat', 12000.00, 18000.00, 'black', 'red', 'X22', 'Mastercraft', 'new', 5, '2020-05-01 12:30:00', 2019,
        '2020-04-12 16:30:00', 5);
UNLOCK
TABLES;
...
VALUES ('ccb', 'Aircraft', 5750000.00, 5760000.00, 'white and blue', 'white', 'XLS+', 'Cessna', 'used', 9,
        '2021-04-18 12:30:00', 2019, '2021-04-12 16:30:00', 5);
UNLOCK
TABLES;

LOCK
TABLES aircraft WRITE;
...
LOCK
TABLES bid WRITE;
INSERT INTO bid (listingID, bidder, bidValue)
VALUES (4, 6, 200.00),
       (1, 3, 210.00),
       (1, 4, 220.00),
       (2, 2, 210.00),
...
SELECT SUM(minSellPrice) AS `Total Earnings`
FROM auctionItem;
#
Earnings Per Item
SELECT manufacturer AS Manufacturer, model AS Model, COUNT(model) AS Quantity, SUM(minSellPrice) AS Earnings
FROM auctionItem
GROUP BY manufacturer, model;
#
Earnings Per Item Type
SELECT type AS `Item Type`, SUM(minSellPrice) AS `Earnings`
FROM auctionItem
GROUP BY type;
#
Earnings Per
...
SELECT a.first_name         AS `First Name`,
       ...
       a.username,
       SUM(aI.minSellPrice) AS `Total Money Spent`
FROM account a
         INNER JOIN auctionItem aI
                    ON a.account_number = aI.purchaser
GROUP BY aI.purchaser
ORDER BY `Total Money Spent` DESC LIMIT 5;